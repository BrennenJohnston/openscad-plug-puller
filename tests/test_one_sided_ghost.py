...
* the switch lives in Step 1 of the Customizer, on by default;
* exports are identical with the switch on and off;
...
STANDARD_PLUG = {"plug_preset": "Standard 3-prong plug - NEMA 5-15"}
...
def _export(runner, out: Path, show: bool) -> str:
    ...
    for name, value in {**STANDARD_PLUG, "show_plug_preview": show}.items():
        cmd += _define(name, value)
    cmd.append(str(ONE_SIDED_SCAD))
